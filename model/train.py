...
class GAN(L.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer_G = torch.optim.NAdam(self.generator.parameters(), lr=self.hparams.lr)
        optimizer_D = torch.optim.NAdam(self.discriminator.parameters(), lr=self.hparams.lr)
        
        scheduler_G = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_G, mode='min', factor=0.1, 
                                                               patience=1, eps=1e-20)
        scheduler_D = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_D, mode='min', factor=0.1, 
                                                               patience=3, eps=1e-20)
        return [
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_dir', type=str, default='../data/train_gan_cube_data')
    parser.add_argument('--val_dir', type=str, default='../data/val_gan_cube_data')

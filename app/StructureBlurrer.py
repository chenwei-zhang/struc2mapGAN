...
def write_mrc_file(data, origin, voxel_size, mrc_file):
    ...
    Returns:
        None
    """
    with mrcfile.new(f'{mrc_file}_pdb2vol.mrc', overwrite=True) as mrc:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pdb", help="Path to the input PDB or CIF file.")
    parser.add_argument("--res", type=float, help="Resolution of the output map.")
    parser.add_argument("--output_mrc", help="Path to save the output MRC file, don't need suffix '.mrc'.")
    parser.add_argument("-m", "--ref_map", help="BUG, DO NOT USE", default=False)
    ...
    print(f'pdb2vol saved to {args.output_mrc}_pdb2vol.mrc')
